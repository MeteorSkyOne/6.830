...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		// some code goes here
		// If f is null, it finds the left-most leaf page recursively
		if (f == null) {
	...
	public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		if (page.getRightSiblingId() != null){
			// already have right sibling
			// then do things like page -> newRight -> originRight
			BTreeLeafPage originRightPage = (BTreeLeafPage) getPage(tid, dirtypages, page.getRightSiblingId(), Permissions.READ_WRITE);
			originRightPage.setLeftSiblingId(rightPage.getId());
			rightPage.setRightSiblingId(originRightPage.getId());
			dirtypages.put(originRightPage.getId(), originRightPage);
		}

		page.setRightSiblingId(rightPage.getId());
		rightPage.setLeftSiblingId(page.getId());

		// half left in the left and half to be added to right
		int half = numTuples / 2;
		Iterator<Tuple> it = page.reverseIterator();
		Field key = null;
		while (half > 0) {
			half--;
			Tuple next = it.next();
			key = next.getField(page.keyField);
	...
	public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// key that needs to push to parent
		BTreeEntry keyEntry = it.next();
		page.deleteKeyAndRightChild(keyEntry);

		keyEntry.setLeftChild(page.getId());
		keyEntry.setRightChild(rightPage.getId());
		...
		BTreeInternalPage parentPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), keyEntry.getKey());

		parentPage.insertEntry(keyEntry);
		dirtypages.put(parentPage.getId(), parentPage);
		updateParentPointers(tid, dirtypages, parentPage);

		return field.compare(Op.LESS_THAN, keyEntry.getKey()) ? page : rightPage;
	}
	...
	public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int stealCount = siblingNumTuples - (pageNumTuples + siblingNumTuples) / 2;

		Iterator<Tuple> it = isRightSibling ? sibling.iterator() : sibling.reverseIterator();
		Tuple lastTuple = null;
		while (stealCount > 0 && it.hasNext()) {
			lastTuple = it.next();
			sibling.deleteTuple(lastTuple);
			page.insertTuple(lastTuple);
			stealCount--;
		}
		entry.setKey(lastTuple.getField(sibling.keyField));
	...
	public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		if (siblingNumEntries <= pageNumEntries) return;

		int stealCount = siblingNumEntries - (pageNumEntries + siblingNumEntries) / 2;
		...
		stealCount--;
		// insert an entry with parentEntry's key to page
